...
function setPlayerNames() {
    var xesPlayerName = $('#xesPlayerName').val();
    var osPlayerName = $('#osPlayerName').val();

    if (xesPlayerName === '' || osPlayerName === '') {
        sweetAlert("Oops...", "Both Player's must have a name!", "error");

        readyTheBoard(0);
    } else {
        // push names to their places
        $('.nameOne').text(xesPlayerName);
        $('.nameTwo').text(osPlayerName);
...
function readyTheBoard(state) {
    if (state === 1) {
        ...
        // reset the board
        $('.tickBoard div').removeClass('taken playerOne playerTwo').html('');
...
function testScore() {
    ...
    // Test for tie game
    if ($('.tickBoard div.taken').length === 9) {
        tieGame();
    }
}
